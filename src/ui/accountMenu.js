...
const MENU_TITLES = Object.freeze({
  ...
  signup: ["Регистрация", "Почта, пароль и имя пользователя."],
  reset: ["Пароль", "Пришлём ссылку для смены пароля на почту."],
  profile: ["Имя пользователя", "По нему вас найдут друзья."],
...
function renderLinks(mode) {
  ...
  const hint = mode === "signup"
    ? "<small>На почту придёт письмо с подтверждением.</small>"
    : "";
