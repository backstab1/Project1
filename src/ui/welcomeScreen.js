...
const FEATURES = [
  ...
  ["archive", "Библиотека в аккаунте",
    "Записи живут на сервере и открываются с любого устройства. Обратимое удаление — на случай неверного клика."],
];
...
const FAQ = [
  ...
  ["Как завести аккаунт?",
    "Почта, пароль и имя пользователя. Подтвердите почту по ссылке из письма — и библиотека ваша."],
  ...
  ["Как наполнить библиотеку?",
    "Через поиск: наберите название, выберите карточку — год, длительность, страна, жанры, описание и постер подставятся сами."],
  ["Откуда берутся данные о фильмах?",
    "Из TMDB. Подключать ничего не нужно: сервис ходит туда сам, а постеры отдаёт их же CDN."],
  ...
  ["Это готовый релиз?",
    `Текущая версия — ${APP_VERSION}: основной цикл собран полностью, статус пока бета, регистрация открыта.`],
];
    ...
    <div class="wl">
      ...
      <section class="wl-hero">
        ...
        <div class="wl-hero__inner">
          ${badge("Открытая бета · регистрация свободная", "lock")}
          ...
          <p class="wl-meta">
            <span><i class="wl-dot"></i>Аккаунт за минуту</span>
      ...
      <section class="wl-sec wl-sec--tight">
        <div class="wl-metrics">
          ${empty ? `
            <article class="wl-metric"><b>1</b><span>минута до библиотеки — почта, пароль и имя пользователя</span></article>
            <article class="wl-metric"><b>0,5</b><span>шаг оценки — половинки звёзд, без округлений в чужую пользу</span></article>
            <article class="wl-metric"><b>500</b><span>фильмов в библиотеке — рабочий размер, под который всё считано</span></article>
          ` : `
      ...
      <section class="wl-sec" id="welcome-account">
        <header class="wl-head">
          ...
          <p>CineVault — сервис с личным кабинетом. Вход и регистрация по почте
          и паролю: библиотека остаётся видна только вам.</p>
        </header>
        <div class="wl-cards">
          <article class="wl-card">
            ...
            <ul class="wl-list">
              ...
              ${tick("Библиотека открывается с любого устройства, где вы вошли.")}
            </ul>
          </article>
          <article class="wl-card">
            ...
            <ul class="wl-list">
              ...
              ${tick("Имя пользователя, по которому вас найдут друзья.")}
      ...
      <section class="wl-final">
        ...
        <p>${locked
          ? "До библиотеки осталась одна форма: почта, пароль и имя пользователя."
